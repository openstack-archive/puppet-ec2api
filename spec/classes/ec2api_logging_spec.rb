...
describe 'ec2api::logging' do
  ...
  let :log_params do
    {
     ...
     :log_dir                        => '/var/log',
     :log_file                       => '/var/tmp/ec2api_random.log',
     :watch_log_file                 => true,
     :debug                          => true,
    }
  end
  ...
  shared_examples 'basic default logging settings' do
    it 'configures ec2api logging settings with default values' do
      is_expected.to contain_oslo__log('ec2api_config').with(
  ...
  shared_examples 'basic non-default logging settings' do
    it 'configures ec2api logging settings with non-default values' do
      is_expected.to contain_oslo__log('ec2api_config').with(
        :use_syslog          => true,
        :use_json            => true,
        :use_journal         => true,
  ...
  shared_examples_for 'logging params unset' do
   [ :logging_context_format_string, :logging_default_format_string,
     ...
     :log_date_format, ].each { |param|
        it { is_expected.to contain_oslo__log('ec2api_config').with("#{param}" => '<SERVICE DEFAULT>') }
      }
  end
  ...
  on_supported_os({
    ...
    context "on #{os}" do
      ...
      it_behaves_like 'ec2api-logging'
    end
  end
end
