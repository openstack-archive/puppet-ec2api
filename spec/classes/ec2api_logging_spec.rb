...
describe 'ec2api::logging' do
  ...
  shared_examples 'basic default logging settings' do
    it 'configures ec2api logging settins with default values' do
      is_expected.to contain_ec2api_config('DEFAULT/use_syslog').with(:value => '<SERVICE DEFAULT>')
      is_expected.to contain_ec2api_config('DEFAULT/use_stderr').with(:value => '<SERVICE DEFAULT>')
      is_expected.to contain_ec2api_config('DEFAULT/syslog_log_facility').with(:value => '<SERVICE DEFAULT>')
      is_expected.to contain_ec2api_config('DEFAULT/log_dir').with(:value => '/var/log/ec2api')
      is_expected.to contain_ec2api_config('DEFAULT/log_file').with(:value => '/var/log/ec2api/ec2api.log')
      is_expected.to contain_ec2api_config('DEFAULT/verbose').with(:value => '<SERVICE DEFAULT>')
      is_expected.to contain_ec2api_config('DEFAULT/debug').with(:value => '<SERVICE DEFAULT>')
    end
  end

  shared_examples 'basic non-default logging settings' do
    it 'configures ec2api logging settins with non-default values' do
      is_expected.to contain_ec2api_config('DEFAULT/use_syslog').with(:value => 'true')
      is_expected.to contain_ec2api_config('DEFAULT/use_stderr').with(:value => 'false')
      is_expected.to contain_ec2api_config('DEFAULT/syslog_log_facility').with(:value => 'LOG_FOO')
      is_expected.to contain_ec2api_config('DEFAULT/log_dir').with(:value => '/var/log')
      is_expected.to contain_ec2api_config('DEFAULT/log_file').with(:value => '/var/log/ec2api.log')
      is_expected.to contain_ec2api_config('DEFAULT/verbose').with(:value => 'true')
      is_expected.to contain_ec2api_config('DEFAULT/debug').with(:value => 'true')
    end
  end
  ...
  shared_examples_for 'logging params unset' do
   [ :logging_context_format_string, :logging_default_format_string,
     ...
     :log_date_format, ].each { |param|
        it { is_expected.to contain_ec2api_config("DEFAULT/#{param}").with_value('<SERVICE DEFAULT>') }
      }
  end
